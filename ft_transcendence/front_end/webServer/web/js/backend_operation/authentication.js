...
export async function requestToken(f_log) {
	...
	try {
		const response = await fetch("https://localhost/api/v1/token", {
			method: "POST",
...
export function openSocketClient() {
	const sendData = new dataToServer('connection', client.inforUser, client.inforUser, 'socket server');
	client.socket.send(JSON.stringify(sendData));
}

export function closeSocketClient() {
	const sendData = new dataToServer('disconnection', client.inforUser, client.inforUser, 'socket server');
	client.socket.send(JSON.stringify(sendData));
}
